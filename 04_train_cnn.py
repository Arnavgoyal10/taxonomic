"""
1D Convolutional Neural Network for Taxonomic Classification
============================================================
Uses character-level embeddings and 1D-CNN architecture for sequence classification.
Implements multi-scale convolutions with different kernel sizes.
...
class SequencePreprocessor:
    """
    Preprocess protein sequences for CNN input.
    """
...
class CNNTaxonomyClassifier:
    """
    1D-CNN classifier for taxonomic prediction.
    """
    ...
    def build_model(self):
        ...
        logger.info(f"  CNN filters: {config.CNN_FILTERS}")
        logger.info(f"  Kernel sizes: {config.KERNEL_SIZES}")
        ...
        conv_branches = []
        for i, (filters, kernel_size) in enumerate(zip(config.CNN_FILTERS, config.KERNEL_SIZES)):
            # Conv layer
            conv = layers.Conv1D(
                filters=filters,
                ...
                padding='same',
                activation='relu',
                name=f'conv_{i+1}_k{kernel_size}'
            )(embedding)
        ...
        if len(conv_branches) > 1:
            concatenated = layers.Concatenate(name='concatenate')(conv_branches)
        else:
            concatenated = conv_branches[0]
        
        # Dense layers
        dense = layers.Dense(512, activation='relu', name='dense_1')(concatenated)
        dense = layers.BatchNormalization(name='bn_dense_1')(dense)
        dense = layers.Dropout(config.DROPOUT_RATE, name='dropout_1')(dense)
        
        dense = layers.Dense(256, activation='relu', name='dense_2')(dense)
        dense = layers.BatchNormalization(name='bn_dense_2')(dense)
        dense = layers.Dropout(config.DROPOUT_RATE, name='dropout_2')(dense)
        ...
        # Create model
        model = models.Model(inputs=input_layer, outputs=output, name=f'cnn_{self.target}')
    ...
    def train(self, X_train, y_train, X_val, y_val):
        ...
        model_checkpoint_path = os.path.join(
            ...
            f'cnn_{self.target}_{timestamp}_best.h5'
        )
        ...
        callback_list = [
            ...
            callbacks.TensorBoard(
                log_dir=os.path.join('logs', f'cnn_{self.target}_{timestamp}'),
                histogram_freq=1
            )
        ]
    ...
    def plot_training_history(self):
        ...
        plot_path = os.path.join(
            ...
            f'training_history_cnn_{self.target}_{timestamp}.png'
        )
    ...
    def evaluate(self, X_test, y_test_cat, y_test):
        ...
        # Evaluate
        test_loss, test_accuracy, test_top3 = self.model.evaluate(X_test, y_test_cat, verbose=1)
    ...
    def plot_confusion_matrix(self, cm, class_names):
        ...
        plt.title(f'Confusion Matrix - CNN ({self.target.capitalize()})')
        ...
        plot_path = os.path.join(
            config.PLOTS_OUTPUT_DIR,
            f'confusion_matrix_cnn_{self.target}_{timestamp}.png'
        )
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    ...
    def save_model(self):
        ...
        model_path = os.path.join(
            config.MODEL_OUTPUT_DIR,
            f'cnn_{self.target}_{timestamp}.h5'
        )
        ...
        metadata_path = os.path.join(
            config.MODEL_OUTPUT_DIR,
            f'cnn_{self.target}_{timestamp}_metadata.pkl'
        )
    ...
    def save_results(self, metrics):
        ...
        results_path = os.path.join(
            ...
            f'results_cnn_{self.target}_{timestamp}.txt'
        )
        
        with open(results_path, 'w') as f:
            f.write(f"CNN Classifier - {self.target.capitalize()} Prediction\n")
            f.write("="*60 + "\n\n")
            ...
            f.write(f"  Kernel sizes: {config.KERNEL_SIZES}\n")
    ...
    def run_pipeline(self):
        ...
        logger.info("="*60)
        logger.info(f"CNN Training Pipeline - {self.target.upper()}")
        logger.info("="*60)
        ...
        # Prepare data
        X_train, X_val, X_test, y_train, y_val, y_test_cat, y_test = self.prepare_data(sequences, labels)
...
def main():
    ...
    order_classifier = CNNTaxonomyClassifier(target='order')
    ...
    class_classifier = CNNTaxonomyClassifier(target='class')
